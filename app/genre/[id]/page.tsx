...
import { IGenreInfo, ISimilarGenre } from "@/app/types";
...
export default async function Genre({ params,
  ...
  const same_genre: ISimilarGenre | undefined = genre_info.similar_genres.filter(g => g.id == genre_info.genre.id).at(0);
    ...
    <section className="flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-wrap gap-1">
          {
            genre_info.similar_genres.filter(g => g.id != genre_info.genre.id).map(g => <GenreCard key={`card-${g.id}`} id={g.id} title={`${g.name} (${(g.count / (same_genre?.count || 1) * 100).toFixed(2)}%)`}></GenreCard>)
          }
        </div>
      </div>
      <div>
